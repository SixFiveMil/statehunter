# StateHunter: Runtime SPA State Inspector & DOM Sink Auditor (Chrome DevTools)
...
A **Chrome Manifest V3 DevTools Extension** designed for application security engineers, penetration testers, and software developers. Authored by **Joshua A. Wortz, CISSP** ([Code & Cypher](https://codeandcypher.com)).

> 📢 **Call for Peer Review**: We have published **[RFC-001: Community Peer Review](docs/rfcs/RFC-001-COMMUNITY-PEER-REVIEW.md)** and the companion technical paper **[Bridging the Browser-to-Boundary Gap](https://codeandcypher.com/posts/client-side-spa-recon-and-safe-harbor-verification/)**. We invite AppSec engineers, penetration testers, and security researchers to test our de-obfuscation heuristics and share feedback in [GitHub Discussions](https://github.com/SixFiveMil/statehunter/discussions) or [Issues](https://github.com/SixFiveMil/statehunter/issues).

StateHunter performs client-side runtime JavaScript analysis, detects sensitive data exposure in client storage, audits and replays `postMessage` handlers, de-obfuscates hidden Single Page Application (SPA) routes, parses RFC 9116 vulnerability disclosure policies, enforces client-side scope boundaries during active testing, and exports structured audit reports directly into Markdown or [AuditGuard](https://github.com/SixFiveMil/auditguard) format.

---

## 🧪 Automated Test Suite (57 Tests)

StateHunter includes a complete automated test suite powered by [Vitest](https://vitest.dev/), verifying scope boundary enforcement, SPA route extraction, `postMessage` security auditing, and secret scanning heuristics:
...
* **Scope Guardrail Enforcement:** Blocks network requests to excluded paths (`/logout`, `/delete`, `/billing`, or custom exclusions) and out-of-scope targets before `fetch()` is dispatched, labeling them `🛡️ BLOCKED (Scope)`.
* **Route Categorization:** Categorizes routes into `admin`, `api`, `dynamic`, and `standard`.
...
* **DOM Sink Correlation:** Detects cross-origin messages that feed directly into DOM sinks (`innerHTML`, `eval`, `location.href`).
...
* **State & Storage Inspection:** Audits `window.*` globals, `__NEXT_DATA__` hydration props, `localStorage`, and `sessionStorage`.
...
* **JWT Claim Inspector:** Decodes JSON Web Tokens, flagging token expiration and sensitive claims (`role: admin`, `email`, `ssn`).
* **Shannon Entropy Scoring:** Identifies unlabeled high-entropy API keys ($H \ge 4.6$ bits).

### 4. RFC 9116 Policy Parser & In-Browser Scope Guardrails
...
* **Precision Scope Enforcement (Literal vs. Wildcard):** Literal paths (e.g. `/billing`, `/logout`, `/delete`) strictly protect exact routes and direct subpaths. Arbitrary nested path segments require explicit wildcards (e.g. `*/billing/*`, `*delete*`), ensuring testers can block specific literal endpoints without collateral over-blocking. Rules persist in `chrome.storage.local`.

### 5. Audit Reporting & AuditGuard Bridge
* **Markdown Security Audit Report:** Generates formatted audit reports with executive summaries, risk matrices, and PoC reproductions.
...
StateHunter is a DevTools-activated extension compliant with Chrome Web Store Developer Program Policies. When DevTools is closed, zero scripts are injected and zero background telemetry is gathered. When DevTools is opened, StateHunter dynamically injects probes into the inspected tab:
...
* [**Scope & Policy Discovery Guide**](docs/SCOPE_AND_RULES.md): RFC 9116 `security.txt`, `robots.txt` parsing, and AuditGuard integration.
...
StateHunter is authored and maintained by **Joshua A. Wortz, CISSP** at [Code & Cypher](https://codeandcypher.com) — security research, developer tooling, and compliance automation for modern web applications.
